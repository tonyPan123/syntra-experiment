...
if [ "$4" = "trace" ]; then
    timeout 10m mm-delay 25 mm-link $2 $3  -- bash -c "export LD_LIBRARY_PATH=.; sleep 3 ; ./peerconnection_serverless sender.json > sender.log 2>&1" &
    sleep 1
    timeout 10m ./peerconnection_serverless receiver.json > receiver.log 2>&1 &
else 
    timeout 3m mm-delay 25 mm-link $2 $3  -- bash -c "export LD_LIBRARY_PATH=.; sleep 3 ; ./peerconnection_serverless sender.json > sender.log 2>&1" &
    sleep 1
