...
if [ "$4" = "trace" ]; then
    timeout 10m mm-delay 25 mm-link $2 $3  -- bash -c "sleep 10 ; ./peerconnection_gcc sender.json > sender.log 2>&1" &
    sleep 1
    timeout 10m ./peerconnection_gcc receiver.json > receiver.log 2>&1 &
else 
    if [ "$4" = "shallow" ]; then
        timeout 3m mm-delay 25 mm-link $2 $3 --uplink-queue=droptail --uplink-queue-args="bytes=10500"  -- bash -c "sleep 10 ; ./peerconnection_gcc sender.json > sender.log 2>&1" &
    else 
        timeout 3m mm-delay 25 mm-link $2 $3  -- bash -c "sleep 10 ; ./peerconnection_gcc sender.json > sender.log 2>&1" &
    fi
    sleep 1
