...
def main(args):
    ...
    if NM == CCAC:
        ...
        c = Ideal.Config(T=args.tsteps, use_loss_detect=args.use_loss_detect, infinite_buffer=True, N=2, use_ecn=True, use_hop_delay=True)
    ...
    # fix a trace
    trace = []
    # ECN better. Gives upper bound.
    # trace.append(vb.ecn[2])
    # trace.append(z3.Not(vb.hop_delay[2][1]))

    trace.append(vb.C == 25)
    trace.append(vb.hop_delay[4][4])
    for t in range(c.T):
        trace.append(z3.Not(vb.ecn[t]))

    # trace.append(vb.A[2] >= 100 + vb.S[3])
    # for t in range(1, c.T):
    #     trace.append(vb.A[t] - vb.A[t-1] == 100)
    #     # clb.append(vb.I[t] - vb.I[t-1] == 100)
    #     trace.append(vb.S[t] - vb.S[t-1] == 100)
    ...
    print(s.check())
    m = s.model()
    print(NM.get_str(c, vb, m))

    print("Using ECN:")
    c.use_hop_delay = False
    ecn_clb = NM.Constraints().all_constraints(c, vb)
    trace_ecn = []
    for t in range(c.T):
        trace_ecn.append(vb.ecn[t] == m.eval(vb.ecn[t]))
        trace_ecn.append(vb.A[t] == m.eval(vb.A[t]))
        trace_ecn.append(vb.S[t] == m.eval(vb.S[t]))
    g_ecn = z3.Goal()
    ecn_clb.extend(trace_ecn)
    g_ecn.add(z3.Exists(flatten([vb.W, vb.T0]), z3.And(ecn_clb)))
    ret_ecn = qe_simplify(g_ecn)
    print(ret_ecn)

    # import ipdb; ipdb.set_trace()

    print("Using hop delay:")
    c.use_hop_delay = True
    c.use_ecn = False
    hd_clb = NM.Constraints().all_constraints(c, vb)
    trace_hd = []
    for t in range(c.T):
        for dt in range(c.T):
            trace_hd.append(vb.hop_delay[t][dt] == m.eval(vb.hop_delay[t][dt]))
        trace_hd.append(vb.A[t] == m.eval(vb.A[t]))
        trace_hd.append(vb.S[t] == m.eval(vb.S[t]))
    g_hd = z3.Goal()
    hd_clb.extend(trace_hd)
    g_hd.add(z3.Exists(flatten([vb.W, vb.T0]), z3.And(hd_clb)))
    ret_hd = qe_simplify(g_hd)
    print(ret_hd)

    # import ipdb; ipdb.set_trace()

    print("Using only e2e:")
    c.use_hop_delay = False
    c.use_ecn = False
    none_clb = NM.Constraints().all_constraints(c, vb)
    trace_none = []
    for t in range(c.T):
        trace_none.append(vb.A[t] == m.eval(vb.A[t]))
        trace_none.append(vb.S[t] == m.eval(vb.S[t]))
    g_none = z3.Goal()
    none_clb.extend(trace_none)
    g_none.add(z3.Exists(flatten([vb.W, vb.T0]), z3.And(none_clb)))
    ret_none = qe_simplify(g_none)
    print(ret_none)
    ...
    if NM == CCAC:
        g.add(z3.Exists(flatten([vb.W, vb.T0]), z3.And(clb)))
    elif NM == CBRDelay:
